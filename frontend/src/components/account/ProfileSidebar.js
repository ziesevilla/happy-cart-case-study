 ...
 * * NOW: Filters out 'Cancelled' orders from Stats (Count and Total Spent).
 */
const ProfileSidebar = ({ 
    ...
    // --- 💡 FIX: FILTER OUT CANCELLED ORDERS ---
    // 1. Create a list of orders that are NOT cancelled
    const validOrders = useMemo(() => {
        if (!orders) return [];
        return orders.filter(order => order.status !== 'Cancelled');
    }, [orders]);

    // 2. Calculate Total Spent based ONLY on valid orders
    ...
    // 3. Calculate Order Count based ONLY on valid orders
    ...
    const getAvatarUrl = (isPreview = false) => {
        ...
        if (user.profile_image) {
            return user.profile_image.startsWith('http') 
                ? user.profile_image 
                : `http://localhost:80${user.profile_image}`; 
        }
        return null;
    };
        ...
        <>
                ...
                <div className="profile-avatar-container position-relative">
                    <div 
                        ...
                        style={{ 
                            ...
                            marginTop: '-50px', position: 'relative', zIndex: 10, overflow: 'hidden' 
                        }}
                    >
                        {getAvatarUrl() ? (
                            <img src={getAvatarUrl()} alt="Profile" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                        ) : (
                            <span className="fs-1 fw-bold text-secondary">{user.name?.charAt(0).toUpperCase()}</span>
                        )}
                    </div>
                </div>
            ...
            <Card className="border-0 shadow-sm rounded-4 p-4">
                 ...
                 <div className="d-flex align-items-center mb-3">
                    ...
                    {/* 💡 UPDATED: Uses validOrderCount instead of orderCount prop */}
                    <div><small className="d-block text-muted">Total Orders</small><strong>{validOrderCount} Orders</strong></div>
                 </div>

                 <div className="d-flex align-items-center mb-3">
                    <div className="bg-light p-2 rounded-circle me-3"><DollarSign size={20} className="text-primary"/></div>
                    <div>
                        <small className="d-block text-muted">Total Spent</small>
                        {/* 💡 UPDATED: Uses calculatedTotalSpent */}
                        <strong>₱{calculatedTotalSpent.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong>
                    </div>
                 </div>
                    ...
                    <Form onSubmit={handleSaveProfile}>
                        <div className="text-center mb-4">
                            <div className="d-inline-block position-relative">
                                ...
                                >
                                    {getAvatarUrl(true) ? (
                                        <img src={getAvatarUrl(true)} alt="Preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                    ) : (
                        ...
                        <Row className="g-3">
                            <Col xs={12}>
                                <Form.Group><Form.Label>Full Name</Form.Label><Form.Control type="text" className="rounded-pill" value={profileData.name} onChange={(e) => setProfileData({...profileData, name: e.target.value})} required/></Form.Group>
                            </Col>
                            <Col xs={12}>
                                <Form.Group><Form.Label>Phone Number</Form.Label><Form.Control type="tel" className="rounded-pill" value={profileData.phone} onChange={(e) => setProfileData({...profileData, phone: e.target.value})} /></Form.Group>
                            </Col>
                            <Col md={6}>
                                <Form.Group><Form.Label>Date of Birth</Form.Label><Form.Control type="date" className="rounded-pill" value={profileData.dob} onChange={(e) => setProfileData({...profileData, dob: e.target.value})} /></Form.Group>
                            </Col>
                            <Col md={6}>
                                <Form.Group>
                                    <Form.Label>Gender</Form.Label>
                                    <Form.Select className="rounded-pill" value={profileData.gender} onChange={(e) => setProfileData({...profileData, gender: e.target.value})}>
            ...
            <Modal show={showUpdateConfirmModal} onHide={() => setShowUpdateConfirmModal(false)} centered size="sm">
                <Modal.Body className="text-center p-4">
                    ...
                    <div className="d-grid gap-2">
                        <Button variant="primary" onClick={confirmUpdate} className="rounded-pill fw-bold" disabled={loading}>{loading ? <Spinner animation="border" size="sm" /> : "Yes, Update"}</Button>
